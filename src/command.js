...
const actions = {
  ...
  "format-card": () => {
    readICThenExec(
      i18n("log_msg_start_format_card"),
      i18n("indicator_formatting_ic_card"),
      true,
      "nfc-mfclassic",
      ["f", "A", "u", tempMFDFilePath, tempMFDFilePath, "f"]
    );
  },

  // 输入已知密钥解卡
  "input-keys-read-IC": () => {
    createInputKeysWindow("done-input-keys-read-IC");
  },
  ...
  "hard-nested-config-done": (configs) => {
    if (configs.autoRun) {
      ...
      readICThenExec(
        i18n("lod_msg_start_auto_hard_nested"),
        `${i18n("indicator_doing_hard_nested")} - ${
          totalUnknownKeys - unknownKeyInfo.length + 1
        }/${totalUnknownKeys}`,
        false,
        () => {
  ...
  "run-hard-nested": (configs) => {
    ...
    printStatus(
      `${i18n("indicator_collecting_nonces")}${
        configs.autoRun
          ? ` - ${
              totalUnknownKeys - unknownKeyInfo.length + 1
            }/${totalUnknownKeys}`
          ...
          exec(
            ...
            (value) => {
              ...
              if (i >= 0) {
                ...
                if (
                  unknownKeyInfo[0][0] === sector &&
                  unknownKeyInfo[0][1] === keyType
                )
                  unknownKeyInfo.shift();
              }
            }
          )
            .then(() => {
  ...
  "dict-test-config-done": (configs) => {
    ...
    exec(
      i18n("log_msg_start_test_dictionary"),
      "nfc-mfdict",
      [
        `-s${configs.sector}`,
        `-t${configs.keyType}`,
        `-l${configs.startPosition}`,
        `-d${dictPath}`,
      ],
      (value) => {
        ...
        if (i >= 0) {
          ...
          if (unknownKeyInfo.length === 0) return;
          if (
            unknownKeyInfo[0][0] === configs.sector &&
            unknownKeyInfo[0][1] === configs.keyType
          )
            unknownKeyInfo.shift();
        }
      }
  ...
  // 打开历史密钥
  "open-history-keys": () => {
    console.log("knownKeysFile", knownKeysFile);
    cp.exec(
      `${
        process.platform === "win32" ? "notepad.exe" : "open"
      } "${knownKeysFile}"`
    );
  },
  ...
  "dump-editor-choose-file": (filePath) => {
    ...
    fs.readFile(filePaths, (err, data) => {
      ...
      sentToDumpEditorWindow("binary-data", {
        url: filePaths,
        data: groupedHexData,
      });
    });
  ...
  "dump-comparator-choose-file": (type) => {
    ...
    fs.readFile(filePaths, (err, data) => {
      ...
      sentToDumpComparatorWindow("binary-data", {
        url: filePaths,
        data: groupedHexData,
        type: dumpType,
      });
  ...
  "dump-history": () => {
    fs.mkdir(dumpsFolder, () => {
      const dumpFiles = fs
        .readdirSync(dumpsFolder)
        .filter((file) => file.endsWith(".mfd"));
  ...
  "rename-dump-file": (fileObj) => {
    ...
    fs.rename(
      path.join(dumpsFolder, oldName),
      path.join(dumpsFolder, newName),
      (err) => {
        if (err) throw err;
        updateDumpFiles();
      }
          ...
          if (versionName !== `v${app.getVersion()}`) {
            dialog
              .showMessageBox({
                type: "none",
                buttons: [
                  i18n("dialog_button_go_to_download"),
                  i18n("dialog_button_cancel"),
                ],
                message: i18n("dialog_msg_new_version_found"),
                detail: `${i18n(
                  "dialog_msg_version"
                )}: ${versionName}\n\n${i18n(
                  "dialog_msg_update_logs"
                )}\n${updateLogs}`,
              })
              .then((response) => {
...
function saveKeys(keys) {
  const knownKeys = fs
    .readFileSync(knownKeysFile)
    .toString()
    .match(/[0-9A-Fa-f]{12}/g);
...
function readICThenExec(
  ...
  exec(
    ...
    "nfc-mfdetect",
    isSaveDumpFile
      ? [`-O${tempMFDFilePath}`, `-f${knownKeysFile}`]
      : [`-N`, `-f${knownKeysFile}`],
    (value) => {
      keyInfoStatistic(value);
    },
...
function setNFCConfig() {
  ...
  fs.writeFile(nfcConfigFilePath, content, (err) => {
    if (err) throw err;
    exec(
      i18n("log_msg_start_connect_device"),
      "nfc-list",
      [],
      (value) => {
      ...
      () => {
        sendToMainWindow(
          "setting-nfc-config",
          status.isDeviceConnected ? "success" : "failed"
        );
      }
    )
      .then(() => {
